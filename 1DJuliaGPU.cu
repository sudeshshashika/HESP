...
__global__ void Julia(double* X, double* Y, double* image, unsigned char* ArImg, Complex c_,int N){
    ...
    if((idx<N)){//&&(idy<N))
        for(int i=0;i<2048;++i){  //it shoud run for 2048 by 2048 pixels
            int iters = 0;                     //iteration count                 
            Complex z(X[i],Y[idx]); //definign the complex number
            while(iters<250){
                z = z * z + c_;
                if(z.absolute_val(z) <100)
                {
                    image[idx*N+i] = 10*iters%255; 
                }
                ++iters;
                
                ArImg[(idx*N+i)*3]=image[idx*N+i];
            }
        }
    }
}
